  ...
  <div className="flex items-center gap-1 justify-between">
    ...
    <div className="flex-1 min-w-0 relative mx-1">
      <Input
        ...
        onChange={(e) => setSearchTerm(e.target.value)}
        onKeyPress={(e) => e.key === "Enter" && handleSearch()}
        className="w-full bg-white border-gray-300 rounded-full h-7 text-xs pr-7 pl-2.5"
      />
      <Button
        onClick={() => handleSearch()}
        disabled={loading}
        size="sm"
        className="absolute right-0.5 top-1/2 -translate-y-1/2 rounded-full h-6 w-6 p-0 bg-black text-white text-xs flex items-center justify-center min-w-0"
      >
        <svg
    ...
    <div className="flex items-center gap-1">
      <Button
        ...
        onClick={() => handleReset()}
        size="sm"
        className="bg-white border border-gray-300 rounded-full h-7 w-7 p-0 flex items-center justify-center min-w-0"
      >
      ...
      {user && (
        <>
          <Button
            ...
            size="sm"
            className="bg-black text-white rounded-full h-7 px-1.5 flex items-center gap-0.5 disabled:opacity-50 min-w-0"
          >
            <ShoppingCart className="w-3.5 h-3.5" />
            {selected.length > 0 && (
              <span className="text-[10px] leading-none">{selected.length}</span>
            )}
          </Button>
